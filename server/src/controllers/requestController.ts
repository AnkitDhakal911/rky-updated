...
export const getNearbyRequests = async (req: AuthRequest, res: Response) => {
    try {
        const { bloodType, range } = req.query;
        ...
        // Default to 50km if not provided, otherwise use provided range in km
        const maxDistance = range ? Number(range) * 1000 : 50000;
        ...
        const requests = await BloodRequest.find(query).populate('requester', 'name profileImage');
